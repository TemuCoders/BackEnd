...
public class UserManagementExceptionHandler {
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ResponseEntity<BadRequestResponse> handleConstraintViolation(ConstraintViolationException ex) {
    ...
    public ResponseEntity<BadRequestResponse> handleJsonParse(HttpMessageNotReadableException ex) {
        ...
        if (ex.getCause() instanceof InvalidFormatException ife) {
            String path = ife.getPath().isEmpty() ? "payload" : ife.getPath().get(ife.getPath().size() - 1).getFieldName();
            fieldErrors.put(path, "Invalid value '" + ife.getValue() + "' for type " +
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ResponseEntity<BadRequestResponse> handleMethodNotSupported(HttpRequestMethodNotSupportedException ex) {
        Map<String, String> fieldErrors = new LinkedHashMap<>();
        fieldErrors.put("method", "HTTP method not supported: " + ex.getMethod());
        var body = new BadRequestResponse(
                HttpStatus.BAD_REQUEST.value(),
                HttpStatus.BAD_REQUEST.getReasonPhrase(),
    ...
    private static String extractProperty(ConstraintViolation<?> v) {
        var path = v.getPropertyPath();
        String s = path == null ? "" : path.toString();
        if (s.contains(".")) return s.substring(s.lastIndexOf('.') + 1);
        return s.isBlank() ? "value" : s;
    }
}
